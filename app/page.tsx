    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-8 font-sans">
      ...
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400">Welcome back to your template preview.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((item) => (
          <div key={item.name} className="rounded-xl border border-slate-800 bg-slate-900/50 p-6 shadow-sm">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {/* Chart Box */}
        <div className="md:col-span-2 rounded-xl border border-slate-800 bg-slate-900/50 p-6">
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
          ...
          <p className="text-xs text-slate-400 mb-6">You made 265 sales this month.</p>
          <div className="space-y-6">
            {recentSales.map((sale) => (
              <div key={sale.email} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-9 w-9 rounded-full bg-slate-800 flex items-center justify-center text-xs font-bold text-indigo-400">
                    {sale.initial}
                  </div>
                  <div>
                ...
                <span className="text-sm font-semibold text-white">{sale.amount}</span>
              </div>
            ))}
          </div>
        </div>
